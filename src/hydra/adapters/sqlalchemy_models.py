...
class MarketBarRecord(Base):
    ...
    feature_sets: Mapped[list[FeatureSetRecord]] = relationship(
...
class FeatureSetRecord(Base):
    ...
    market_bar_id: Mapped[UUID] = mapped_column(
        ForeignKey("market_bars.id", ondelete="CASCADE"),
        index=True,
    )
    ...
    market_bar: Mapped[MarketBarRecord] = relationship(back_populates="feature_sets")
    strategy_signals: Mapped[list[StrategySignalRecord]] = relationship(
...
class StrategySignalRecord(Base):
    ...
    feature_set: Mapped[FeatureSetRecord] = relationship(back_populates="strategy_signals")
...
class DecisionRecord(Base):
    ...
    id: Mapped[UUID] = mapped_column(PgUUID(as_uuid=True), primary_key=True, default=uuid4)
    strategy_signal_id: Mapped[UUID] = mapped_column(
    ...
    strategy_signal: Mapped[StrategySignalRecord] = relationship(back_populates="decisions")
    paper_trades: Mapped[list[PaperTradeRecord]] = relationship(
...
class PaperTradeRecord(Base):
    ...
    decision: Mapped[DecisionRecord] = relationship(back_populates="paper_trades")


class PerformanceSnapshotRecord(Base):
